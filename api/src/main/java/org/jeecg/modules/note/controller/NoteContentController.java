...
public class NoteContentController {
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      // Step.1 组装查询条件
      QueryWrapper<NoteContent> queryWrapper = null;
      try {
          String paramsStr = request.getParameter("paramsStr");
          if (oConvertUtils.isNotEmpty(paramsStr)) {
              String deString = URLDecoder.decode(paramsStr, "UTF-8");
              NoteContent noteContent = JSON.parseObject(deString, NoteContent.class);
              queryWrapper = QueryGenerator.initQueryWrapper(noteContent, request.getParameterMap());
          }
      ...
      List<NoteContent> pageList = noteContentService.list(queryWrapper);
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "内容列表");
      mv.addObject(NormalExcelConstants.CLASS, NoteContent.class);
      mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("内容列表数据", "导出人:Jeecg", "导出信息"));
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
          ...
          try {
              List<NoteContent> listNoteContents = ExcelImportUtil.importExcel(file.getInputStream(), NoteContent.class, params);
              for (NoteContent noteContentExcel : listNoteContents) {
                  noteContentService.save(noteContentExcel);
              }
              return Result.ok("文件导入成功！数据行数：" + listNoteContents.size());
          } catch (Exception e) {
