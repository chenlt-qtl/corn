...
public class GameScoreController {
	...
	public Result<GameScore> delete(@RequestParam(name="id",required=true) String id) {
		Result<GameScore> result = new Result<GameScore>();
		GameScore gameScore = gameScoreService.getById(id);
		if(gameScore==null) {
			result.error500("未找到对应实体");
		}else {
			boolean ok = gameScoreService.removeById(id);
			if(ok) {
				result.success("删除成功!");
			}
		}
	...
	public Result<GameScore> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		Result<GameScore> result = new Result<GameScore>();
		if(ids==null || "".equals(ids.trim())) {
			result.error500("参数不识别！");
		}else {
			this.gameScoreService.removeByIds(Arrays.asList(ids.split(",")));
			result.success("删除成功!");
		}
		return result;
	}
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      // Step.1 组装查询条件
      QueryWrapper<GameScore> queryWrapper = null;
      try {
          String paramsStr = request.getParameter("paramsStr");
          if (oConvertUtils.isNotEmpty(paramsStr)) {
              String deString = URLDecoder.decode(paramsStr, "UTF-8");
              GameScore gameScore = JSON.parseObject(deString, GameScore.class);
              queryWrapper = QueryGenerator.initQueryWrapper(gameScore, request.getParameterMap());
          }
      ...
      List<GameScore> pageList = gameScoreService.list(queryWrapper);
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "分数列表");
      mv.addObject(NormalExcelConstants.CLASS, GameScore.class);
      mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("分数列表数据", "导出人:Jeecg", "导出信息"));
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
          ...
          try {
              List<GameScore> listGameScores = ExcelImportUtil.importExcel(file.getInputStream(), GameScore.class, params);
              for (GameScore gameScoreExcel : listGameScores) {
                  gameScoreService.save(gameScoreExcel);
              }
              return Result.ok("文件导入成功！数据行数：" + listGameScores.size());
          } catch (Exception e) {
